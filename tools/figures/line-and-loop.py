...
o.append('<defs><marker id="lah2" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" '
         'orient="auto-start-reverse"><path d="M0 1 L8 5 L0 9 Z" fill="#0F6FDB"/></marker>'
...
for i, s in enumerate(STAGES):
    y = y0 + i * (BH + GAP)
    o.append(f'<rect x="{x0}" y="{y}" width="{BW}" height="{BH}" fill="#E9F2FF" stroke="#0F6FDB" stroke-width="1.5"/>')
...
for i, s in enumerate(STAGES):
    ...
    pts.append((px, py))
    o.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r}" fill="#E9F2FF" stroke="#0F6FDB" stroke-width="1.5"/>')
...
for i in range(6):
    ...
    o.append(f'<path d="M {x1:.1f} {y1:.1f} A {R} {R} 0 0 1 {x2:.1f} {y2:.1f}" fill="none" stroke="#0F6FDB" '
             'stroke-width="5" stroke-linecap="round" marker-end="url(#lah2)"/>')

o.append(f'<circle cx="{cx}" cy="{cy}" r="86" fill="#0F6FDB"/>')
o.append(f'<text x="{cx}" y="{cy+7}" text-anchor="middle" font-family="ui-monospace,\'JetBrains Mono\',Menlo,monospace" '
         'font-size="19" fill="#FFFFFF">agents</text>')
for i, s in enumerate(STAGES):
    px, py = pts[i]
